...
// the skill-runs tile counts the trailing 24h; the avg-diagnose tile averages a
// wider recent window so a single fast/slow run doesn't whip the number around.
const SKILL_RUNS_WINDOW_MS = 24 * 60 * 60 * 1000;
const AVG_DIAGNOSE_WINDOW_MS = 7 * 24 * 60 * 60 * 1000;
...
// better-sqlite3 is synchronous, so no await ceremony. explicit @Inject token (lessons.md).
@Injectable()
export class OverviewService {
  ...
  private avgDiagnoseMs(userId: string): null | number {
    const cutoff = new Date(Date.now() - AVG_DIAGNOSE_WINDOW_MS);
    const rows = this.db
  ...
  private skillRuns24h(userId: string): OverviewMetrics['skillRuns24h'] {
    const cutoff = new Date(Date.now() - SKILL_RUNS_WINDOW_MS);
    const rows = this.db
