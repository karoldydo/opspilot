...
- **`git`** — for staging, the break-check restore, one commit per phase, and a final epilogue commit closing the plan out. The commits land on whatever branch is checked out; `/odp-plan` is what puts you on the change's own branch, but this skill does not require that and never switches branches itself.
...
- **Fixed narration tokens** — `GATE <name>: PASS`, `COMMIT p<N>: <sha>`, `DIRTY (not staged):`, `ADAPT:`, `STOPPED —`, `RUN REPORT —`. The label is a token; the text after it is Polish.
...
- **Never push, never rebase, never rewrite history, never switch branches.** Whatever branch is checked out is where the commits land.
...
1. **Resolve the plan**:
   ...
   **Before concluding the plan does not exist, check for a worktree.** A change opened with `/odp-plan`'s worktree option keeps its whole folder in that other directory, so from here the path is simply absent — which looks identical to a change-id that was never opened. Run:
...
2. **Check you are in the right working tree.** Read `worktree:` from `.context/changes/<change-id>/change.md`. If it is `null`, absent, or equal to the current repository root (`git rev-parse --show-toplevel`), continue. Otherwise `/odp-plan` opened this change in a **separate git worktree** and you are standing somewhere else — the branch with the change's commits is checked out there, not here. Print and STOP:
...
1. **Check empty diff**: `git diff --cached --quiet`. Exit code 0 means nothing to commit — print `Phase <N> had no diff to commit; rows remain SHA-less; /odp-archive surfaces them as a warning.`, set `SHA=""`, and skip to step 5.
...
5. **Write the SHA back into Progress**: for every row flipped during this phase, Edit `- [x] N.M <title>` → `- [x] N.M <title> — <SHA>`. Skip rows that already carry a SHA suffix (resume safety — never double-append). If `SHA=""`, leave the rows SHA-less; `/odp-archive` surfaces them as an informational warning, not an error.
...
git reset --hard <sha of the phase before> # discards, only on a branch nobody else has
```

Surface `git revert` first when a human asks. `reset --hard` also throws away the Progress rows and the `change.md` stamp that rode along with the phase, which is usually not what someone wants.
...
Epilogue: <sha>   (plan close-out — not a step)
Roll back a phase: git revert --no-edit <sha>
